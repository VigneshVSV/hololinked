...
and/or the network provides access to the hardware for multiple use cases in a client-server model. Python objects visible 
...
``Thing``. The supplied ``URL_path`` and HTTP request methods to the actions and properties are used by this HTTP server: 
...
|br|
Supplying a non-string (except for None as ``allow_None`` was set to ``True``) will cause a ``ValueError``. 
