...
class TestSerializer(TestCase):
    ...
    def test_2_protocol_registration(self):
        ...
        self.assertEqual(Serializers().base, base_serializer) # we dont support getitem at instance level yet
        ...
        self.assertEqual(Serializers().yaml, yaml_serializer) # we dont support getitem at instance level yet
    ...
    def test_3_registration_for_objects(self):
        ...
        # get existing number of serializers
        num_serializers = len(Serializers.object_content_type_map)
        
        # test register a new serializer for an object
       


    def test_4_registration_for_objects_by_name(self):
        
        Serializers.register_content_type_for_object_by_name('test_thing', 'test_property', 'application/yaml')
        ...
        self.assertEqual(Serializers.object_content_type_map['test_thing']['test_property'], 'application/yaml')
        self.assertIsInstance(Serializers.for_object('test_thing', 'test_property'), self.__class__.YAMLSerializer)
    
   
    def test_5_retrieval(self):
        # added in previous test
        self.assertIsInstance(Serializers.for_object('test_thing', 'test_property'), self.__class__.YAMLSerializer)
        # unknown object should retrieve the default serializer
        self.assertEqual(Serializers.for_object('test_thing', 'test_unknown_property'), Serializers.default)  
        # unknown thing should retrieve the default serializer
        self.assertEqual(Serializers.for_object('test_unknown_thing', 'test_property'), Serializers.default)


    def test_6_set_default(self):
        ...
        self.test_5_retrieval() # check if retrieval is consistent with default
